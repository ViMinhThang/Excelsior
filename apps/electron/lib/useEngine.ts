...
export function useEngine(engineUrl: string) {
  ...
  useEffect(() => {
    ...
    const client = new EngineClient(engineUrl, {
      onMessage: (msg) => {
        if (alive) dispatch({ type: "server", msg });
      },
      onStatus: (status) => {
        if (alive) dispatch({ type: "status", status });
      },
      onError: (message) => {
        if (alive) dispatch({ type: "error", message });
      },
      getToken: async (url) =>
        sessionStorage.getItem("engine-token:" + url) ||
        (await window.electronAPI?.getEngineToken(url)) ||
        "",
    });
  ...
  const perform = useCallback(
    async <T>(
      operation: (client: EngineClient) => Promise<T>,
    ): Promise<T | undefined> => {
      const client = clientRef.current;
      try {
        if (!client) {
          throw new Error("Engine disconnected; command was not sent.");
        }
  ...
  useEffect(() => {
    if (state.refreshList && state.status === "connected")
      void perform((c) => c.refreshSessions());
  ...
  const createSession = useCallback(
    (title?: string) => perform((c) => c.createSession(title)),
    [perform],
  );
  const setWorkspace = useCallback(
    (workspace: string) => perform((c) => c.setWorkspace(workspace)),
    [perform],
  );
  ...
  const cancelRun = useCallback(async () => {
    const s = stateRef.current,
      id = s.activeId;
    if (!id || !s.runIds[id]) return;
    await perform((c) => c.cancelRun(id, s.runIds[id]));
  }, [perform]);
  const replyAsk = useCallback(
    (q: AskReq, selected: number, answer: string, label: string) =>
      perform((c) =>
        c.replyAsk({
  ...
  const replyPermission = useCallback(
    (p: PermissionReq, approved: boolean) =>
      perform((c) =>
        c.replyPermission({
  ...
  const deleteSession = useCallback(
    (id: string) => perform((c) => c.deleteSession(id)),
    [perform],
  );
  const renameSession = useCallback(
    (id: string, title: string) => perform((c) => c.renameSession(id, title)),
    [perform],
  );
  const setSettings = useCallback(
    (settings: Commands["settings.set"]) =>
      perform((c) => c.setSettings(settings)),
    [perform],
  );
  ...
  return {
    ...
    blocks: id ? (state.blocksBySession[id] ?? []) : [],
    streaming: !!(id && state.streamingBySession[id]),
    usage: id ? (state.usageBySession[id] ?? NO_USAGE) : NO_USAGE,
    ask: id ? (state.asksBySession[id] ?? null) : null,
    permission: id ? (state.permsBySession[id] ?? null) : null,
    outcome: id ? (state.outcomes[id] ?? null) : null,
