...
export class RunEventWriter {
  ...
  private readonly messages: MessageComposer;
  ...
  constructor(private readonly emit: HarnessEventEmitter) {
    this.messages = new MessageComposer(this.emit);
  }

  startMessage(id = `msg_${randomUUID()}`): void {
    if (this.assistant) return;
    this.assistant = { id, content: "" };
    this.messages.startMessage(id, {
      role: "assistant",
      content: "",
    });
  }
  ...
  updateMessage(id: string, delta: string): void {
    ...
    this.messages.updateMessage(this.assistant?.id ?? id, delta);
  }
  ...
  emitNotice(content: string): void {
    this.messages.assistantMessage(content);
  }
}
