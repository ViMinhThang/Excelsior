...
import { createGrepTool } from "../agent/tools/fs/grep.js";
...
describe("filesystem tool workspace bounds", () => {
  ...
  it("rejects glob and grep patterns that escape the workspace", async () => {
    const globResult = await (createGlobTool(ctx()) as any).execute({ pattern: "../*.txt" });
    const grepResult = await (createGrepTool(ctx()) as any).execute({ query: "secret", pathPattern: "../**/*" });
